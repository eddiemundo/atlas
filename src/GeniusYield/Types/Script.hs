...
instance IsString GYMintingPolicyId where
  fromString s =
    either (error . show) GYMintingPolicyId $
...
mintingPolicyIdFromText policyid =
  bimap customError mintingPolicyIdFromApi
    $ Api.deserialiseFromRawBytesHex (TE.encodeUtf8 policyid)
 where
...
someScriptToReferenceApi (GYPlutusScript (GYScript v apiScript _)) =
  Api.S.ReferenceScript
    apiBabbageEraOnwards
    $ case v of
      SingPlutusV1 -> Api.ScriptInAnyLang (Api.PlutusScriptLanguage Api.PlutusScriptV1) $ Api.PlutusScript Api.PlutusScriptV1 apiScript
      SingPlutusV2 -> Api.ScriptInAnyLang (Api.PlutusScriptLanguage Api.PlutusScriptV2) $ Api.PlutusScript Api.PlutusScriptV2 apiScript
      SingPlutusV3 -> Api.ScriptInAnyLang (Api.PlutusScriptLanguage Api.PlutusScriptV3) $ Api.PlutusScript Api.PlutusScriptV3 apiScript
...
someScriptFromReferenceApi
  ( Api.S.ReferenceScript
      Api.S.BabbageEraOnwardsConway
      ( Api.ScriptInAnyLang
          (Api.PlutusScriptLanguage Api.PlutusScriptV4)
          _
        )
    ) = Nothing
someScriptFromReferenceApi
  (Api.S.ReferenceScript _ _) = Nothing
...
scriptFromApi script = GYScript v script apiHash
 where
  ...
  apiScript = case v of
    SingPlutusV1 -> Api.PlutusScript Api.PlutusScriptV1 script
    SingPlutusV2 -> Api.PlutusScript Api.PlutusScriptV2 script
    SingPlutusV3 -> Api.PlutusScript Api.PlutusScriptV3 script
...
readScript' bs = case Aeson.eitherDecodeStrict bs of
  ...
  Right (te :: Api.TextEnvelope) -> case singPlutusVersion @v of
    SingPlutusV1 -> scriptFromApi <$> Api.deserialiseFromTextEnvelope te
    SingPlutusV2 -> scriptFromApi <$> Api.deserialiseFromTextEnvelope te
    SingPlutusV3 -> scriptFromApi <$> Api.deserialiseFromTextEnvelope te
...
readScript file = case singPlutusVersion @v of
  SingPlutusV1 -> do
    e <- Api.readFileTextEnvelope (Api.File file)
    case e of
...
anyScriptToApiScriptInEra (GYPlutusScript s@(GYScript v _ _)) = Api.ScriptInEra scriptInLanguageEra (scriptToApiScript s)
 where
  scriptInLanguageEra = case v of
    SingPlutusV1 -> apiPlutusScriptV1InEra
    SingPlutusV2 -> apiPlutusScriptV2InEra
    SingPlutusV3 -> apiPlutusScriptV3InEra

  scriptToApiScript :: GYScript v -> Api.Script (PlutusVersionToApi v)
  scriptToApiScript (GYScript v' api _) = case v' of
    SingPlutusV1 -> Api.PlutusScript Api.PlutusScriptV1 api
    SingPlutusV2 -> Api.PlutusScript Api.PlutusScriptV2 api
    SingPlutusV3 -> Api.PlutusScript Api.PlutusScriptV3 api
anyScriptToApiScriptInEra (GYSimpleScript s) = Api.ScriptInEra apiSimpleScriptInEra (Api.SimpleScript $ simpleScriptToApi s)
