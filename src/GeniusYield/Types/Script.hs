...
someScriptToReferenceApi (GYPlutusScript (GYScript v apiScript _)) =
  Api.S.ReferenceScript
    ...
    $ case v' of
      ...
      Api.PlutusScriptV4 -> Api.ScriptInAnyLang (Api.PlutusScriptLanguage v') $ Api.PlutusScript v' apiScript
 where
...
someScriptFromReferenceApi
  ( Api.S.ReferenceScript
      Api.S.BabbageEraOnwardsDijkstra
      (Api.ScriptInAnyLang Api.SimpleScriptLanguage (Api.SimpleScript s))
    ) = Just $ GYSimpleScript $ simpleScriptFromApi s
someScriptFromReferenceApi
  ( Api.S.ReferenceScript
...
scriptFromApi script = GYScript v script apiHash
 where
  ...
  apiScript = case v' of
    ...
    Api.PlutusScriptV4 -> Api.PlutusScript v' script
...
scriptFromCBOR' b = do
  ...
  case singPlutusVersion @v of
    ...
    SingPlutusV4 -> fmap scriptFromApi $ rightToMaybe $ flip Api.deserialiseFromRawBytes bs $ Api.AsPlutusScript $ Api.proxyToAsType $ Proxy @(PlutusVersionToApi v)
...
readScript' bs = case Aeson.eitherDecodeStrict bs of
  ...
  Right (te :: Api.TextEnvelope) -> case singPlutusVersion @v of
    ...
    SingPlutusV4 -> scriptFromApi <$> Api.deserialiseFromTextEnvelope te
...
writeScriptCore desc file s = do
  e <- case scriptVersion @v s of
    ...
    SingPlutusV4 -> Api.writeFileTextEnvelope (Api.File file) (Just desc) $ scriptToApi s
  case e of
...
anyScriptToApiScriptInEra (GYPlutusScript s@(GYScript v _ _)) = Api.ScriptInEra scriptInLanguageEra (scriptToApiScript s)
 where
  scriptInLanguageEra = case singPlutusVersionToApi v of
    Api.PlutusScriptV1 -> apiPlutusScriptV1InEra
    Api.PlutusScriptV2 -> apiPlutusScriptV2InEra
    Api.PlutusScriptV3 -> apiPlutusScriptV3InEra
    Api.PlutusScriptV4 -> apiPlutusScriptV4InEra
  ...
  scriptToApiScript (GYScript (singPlutusVersionToApi -> v') api _) = case v' of
    ...
    Api.PlutusScriptV4 -> Api.PlutusScript v' api
